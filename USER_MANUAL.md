...
```
用户输入请求 → AI 分析 → 提出工具调用 → 用户批准/拒绝（审批卡片可展开「查看改动」预览 diff，或点「全部批准」自动放行本次会话后续写操作） → 执行 → ... → diff → 用户批准提交 → git commit
```
...
- **改动预览**：审批卡片可展开「查看改动」，批准前先看到将改什么
- **全部批准**：审批卡片上的「全部批准」按钮，批准当前操作并自动批准本次会话后续所有写操作
- **一键撤销**：历史时间线中每条 live-edit 提交带「撤销」按钮，确认后自动回滚该次改动
