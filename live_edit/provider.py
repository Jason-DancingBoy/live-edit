...
class AnthropicCompatibleProvider(Provider):
    ...
    def __init__(
        self, api_url: str, api_key: str, model: str, timeout: int = 180, max_retries: int = 3
    ):
    ...
    async def call_with_tools(
        ...
        for attempt in range(self._max_retries + 1):
            ...
            except (httpx.TransportError, httpx.TimeoutException, httpx.ConnectError, OSError) as e:
                last_error = e
                if attempt < self._max_retries:
                    delay = 2**attempt
                    logger.warning(
